...
def model_feature_importance(X, model_vars, save_plot=False):
    ...
    importance = pd.DataFrame(
        {"features": X.columns, "importance": model_vars["model"].feature_importances_}
...
def plot_actual_vs_model_pred(model_vars, X, y):
    ...
    fold_ct = (
        model_vars["fold"] - 1
    )  # 0, 1, 2, 3, or 4 representing fold 1, 2, 3, 4 or 5 respectively
    ...
    fig.add_scatter(
        y=model_pred, mode="lines", name=f"{model_vars['model_name']} Predicted"
    )
