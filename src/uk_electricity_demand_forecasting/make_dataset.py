...
def read_and_proc_csvdata(file_path):
    ...
    if len(file_path) == 0:
        ...
        file_path = (
            "../../data/raw/uk_electricity_consumption_historic_demand_2009_2024.csv"
        )
    else:
    ...
    cln_df_columns = [
        column_name.replace("embedded_", "").strip().replace("settlement_", "").strip()
    ...
    demand_n_gen_cols = [
        col
        for col in cln_df.columns.tolist()
        if col == "nd"
        or col == "tsd"
