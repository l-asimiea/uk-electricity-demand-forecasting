...
def create_lag_features(df, save_data=False):
    ...
    if save_data:
        save_path = os.path.join(
            project_root, "data/processed/uk_data_fe_processed.pkl"
        )
...
def plot_correlation_matrix(df, title="Correlation Matrix", plot_save=False):
    ...
    fig = px.imshow(
        correlation_matrix, text_auto=True, title=title, labels={"color": "Correlation"}
    ).update_layout(width=650, height=500)
...
# ----------------------------------------------------------
def plot_feature_distribution(df, feature: str, target=None, bins=30, plot_save=False):
    """
...
def plot_seasonal_decompose(
    ...
    """
    plot_title = (
        f"{target_name} Seasonal Decomposition"
    ...
    fig.update_layout(
        height=900, margin={"t": 100}, title_x=0.5, title=plot_title, showlegend=False
    )
...
def plot_acf_and_pacf(series, lags=20):
    ...
    # ACF Plot
    fig_acf = px.scatter(
        acf_df, x="Lag", y="ACF", title="Autocorrelation Function (ACF)"
    )
    ...
    # PACF Plot
    fig_pacf = px.scatter(
        pacf_df, x="Lag", y="PACF", title="Partial Autocorrelation Function (PACF)"
    )
